        ...
        try:
            if user_phone not in pending_mfa_setups:
                self.send_text_message(
                    user_phone,
                    "No MFA setup in progress. Send 'setup mfa' to begin."
                )
                return False
...
Stay secure! 🔒"""
                ...
                conn.rollback()
                self.send_text_message(user_phone, "Error saving setup. Please try again.")
                return False
            finally:
                conn.close()
...
help            - Show this menu"""
    ...
    # Setup MFA
        ...
        bot.send_text_message(user_phone, "Starting MFA setup...\n\nWhat's your username?")
        pending_mfa_setups[user_phone] = {"waiting_for_username": True}
    
    # Check if waiting for username
    elif user_phone in pending_mfa_setups and pending_mfa_setups[user_phone].get("waiting_for_username"):
        username = text.strip()
        ...
        if not user:
            bot.send_text_message(user_phone, f"User '{username}' not found. Please check your username.")
            del pending_mfa_setups[user_phone]
        else:
