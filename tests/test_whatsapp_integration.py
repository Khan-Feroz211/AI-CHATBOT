...
class WhatsAppTester:
    ...
    def test_webhook_verification(self) -> bool:
        ...
        wrong_token = os.environ.get("WHATSAPP_TEST_WRONG_TOKEN", "invalid-token")
        challenge = "test-challenge-123"

        # Test correct token
        result = self.service.verify_webhook_challenge(
            "subscribe", test_token, challenge
        )
        if result == challenge:
        ...
        # Test wrong token
        result = self.service.verify_webhook_challenge(
            "subscribe", wrong_token, challenge
        )
        if result is None:
        ...
        print("curl -X GET 'http://localhost:8000/api/v1/whatsapp/webhook?")
        print("hub.mode=subscribe&")
        print("hub.challenge=TEST_CHALLENGE&")
    ...
    def test_signature_verification(self) -> bool:
        ...
        try:
            ...
            if not self.service.verify_webhook_signature(
                test_body, "sha256=invalidsignature"
            ):
    ...
    def test_sandbox_mode(self) -> bool:
        ...
        try:
            ...
            request = WhatsAppSendRequest(
                to_phone=test_phone, message="🧪 Test message from sandbox mode"
            )
    ...
    def test_message_formatting(self) -> bool:
        ...
        for name, message in test_cases:
            if len(message) <= 4096:
                self.print_success(
                    f"Message formatting test: {name} ({len(message)} chars)"
                )
            else:
    ...
    def test_phone_validation(self) -> bool:
        ...
        for phone, should_pass, description in test_phones:
            try:
                ...
                WhatsAppSendRequest(to_phone=phone, message="Test")
                self.print_success(f"Phone validation: {description} - {phone}")
            except Exception:
                if should_pass:
                    self.print_error(
                        f"Phone validation failed: {description} - {phone}"
                    )
                    all_passed = False
                else:
    ...
    def test_rate_limiting(self) -> bool:
        ...
        if settings.WHATSAPP_SANDBOX_MODE:
            ...
            for i in range(3):
                try:
                    result = self.service.build_outbound_message(
                        WhatsAppSendRequest(
                    ...
                    self.print_error(f"Rate limit test error: {exc}")
                    break
            ...
            self.print_info(
                "Rate limiting configured (Max retries: {})".format(
                    settings.WHATSAPP_MAX_RETRIES
                )
            )
            self.print_info(
                "Backoff time: {} seconds".format(
    ...
    def run_all_tests(self) -> bool:
        ...
        for test_name, test_func in tests:
            try:
                ...
                self.print_error(f"Test exception: {exc}")
        ...
        for test_name, status in results.items():
            if status == "PASSED":
                ...
                print(
                    f"{self.colors['yellow']}⚠ {test_name}: {status}{self.colors['reset']}"
                )

        print(f"\n{self.colors['blue']}Total: {len(results)} tests")
        print(
            f"Passed: {passed}, Failed: {failed}, Errors: {errors}{self.colors['reset']}\n"
        )
